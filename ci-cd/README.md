...
Эта папка содержит все файлы, связанные с деплоем и CI/CD проекта Cerera.
...
- Dockerfile должен запускаться из корня проекта (использует `COPY . .`)
- docker-compose.yml использует относительные пути к папке grafana в корне проекта
- prometheus.yml находится в этой папке и монтируется в контейнер
