...
# Step 2: Fund accounts via faucet
...
for acc in accounts:
    faucet_req = {
        "method": "faucet",
        "jsonrpc": "2.0",
        "id": 1,  # Using part of address as ID for uniqueness
        "params": [
            acc['address'],
            3.75,  # Amount to fund (adjust as needed)
        ]
    }
    r = requests.post("http://localhost:1337/app", json=faucet_req)
    if r.status_code == 200:
        print(f"Faucet funded {acc['address']}: {r.text}")
    else:
        print(f"Faucet failed for {acc['address']}: {r.text}")
    time.sleep(0.1)
