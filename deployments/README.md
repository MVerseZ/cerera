...
В вариантах с несколькими нодами (3, 5, 9, 15 нод) вместе с нодами поднимается **Block Observer** — веб-интерфейс для просмотра блоков. Откройте в браузере: **http://localhost:8080** (или http://localhost:8080/block_observer.html). На странице можно добавлять URL нод (например http://localhost:1337/app, http://localhost:1338/app) и получать блоки по индексу.
...
- **Block Observer** (при запуске 3/5/9/15 нод): http://localhost:8080 — веб-страница для добавления URL нод и получения блоков
